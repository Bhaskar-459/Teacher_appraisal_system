...
//   console.log("Received KPA data:", id);
  ...
  try {
    ...
    if (
      kpaData.administrativeSupport &&
      kpaData.administrativeSupport.seminars
    ) {
      kpaData.administrativeSupport.seminars =
        kpaData.administrativeSupport.seminars.map((seminar) => ({
    ...
    if (kpaData.administrativeSupport && kpaData.administrativeSupport.events) {
      kpaData.administrativeSupport.events =
        kpaData.administrativeSupport.events.map((event) => ({
    ...
    if (
      kpaData.professionalDevelopment &&
      kpaData.professionalDevelopment.publications
    ) {
      kpaData.professionalDevelopment.publications =
        kpaData.professionalDevelopment.publications.map((pub) => ({
          name: pub.name,
          theme: pub.theme,
    ...
    if (!kpa) {
      ...
      kpa.kpaData.professionalDevelopment = {
        ...kpa.kpaData.professionalDevelopment,
        ...kpaData.professionalDevelopment,
      };
      kpa.kpaData.administrativeSupport = {
        ...kpa.kpaData.administrativeSupport,
        ...kpaData.administrativeSupport,
      };
    ...
    res
      .status(500)
      .json({ message: "Error saving KPA data", error: error.message });
  }
};
